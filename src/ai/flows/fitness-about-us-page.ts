...
const AboutUsContentInputSchema = z.object({
  missionStatementKeywords: z
    .string()
    .describe(
      'Keywords to guide the mission statement, such as fitness planning, ease of use, traditional Fitness, and cryptocurrency.'
    ),
  calculationExplanationsRequest: z
    .string()
    .describe('Request for explanation on the logic of the fitness calculations.'),
...
const AboutUsContentOutputSchema = z.object({
  ...
  calculationExplanations: z
    .string()
    .describe('Explanations of the fitness calculations used.'),
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are tasked with generating content for the About Us page of FinanceForward, a fitness calculator website.

Generate a mission statement based on the following keywords: {{{missionStatementKeywords}}}.

Explain the fitness calculations based on: {{{calculationExplanationsRequest}}}.
